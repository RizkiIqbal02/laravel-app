...
@section('container')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Edit Your Product</h1>
    </div>
    <div class="col-lg-8">
        <form action="/dashboard/products/{{ $product->id }}" method="post" enctype="multipart/form-data">
            ...
            <div class="container">
                <div class="form-group">
                  <label for="rangeInput">Total Stock: <span id="rangeValueDisplay">{{ old('stock', $product->stock) }}</span></label>
                  <input type="range" class="form-range" id="rangeInput" min="0" max="100" step="1">
                  <input type="hidden" id="rangeValue" name="stock" value="{{ old('stock', $product->stock) }}">
                  @error('stock')
                  <div class="invalid-feedback">
                      {{ $message }}
                  </div>
